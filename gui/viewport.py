...
class ZetaViewport(QFrame):
    ...
    def toggle_mpr(self, enabled):
        ...
        if enabled:
            self._cached_wl = self.window_level
            self._cached_ww = self.window_width
            ...
            if not self.mpr_loaded and self.current_file_paths:
                ...
                self.window_level = self._cached_wl
                self.window_width = self._cached_ww
                self.update_display()
        else:
            self.view_plane = 'Axial'
    ...
    def on_mpr_finished(self, volume, spacing):
        ...
        if volume is None:
            self.canvas.overlay_data['BL'] = ["MPR Error"]
            self.canvas.update()
            return
            
        self.volume_data = volume
        self.voxel_spacing = spacing
        self.mpr_loaded = True
    ...
    def _render_mpr(self):
        ...
        try:
            ...
            if self.view_plane == 'Axial':
                slice_img = self.volume_data[self.current_index, :, :]
                aspect_ratio = 1.0
            ...
            # ★修正: SimpleITKのデータは既にRescale適用済みなので、
            # 再度 Slope/Intercept を掛けてはいけません。
            # 生データをそのまま Windowing に渡します。
    ...
    def paging_drag(self, dy):
        ...
        if abs(self.drag_accumulator) > 15:
            ...
            if steps != 0:
                ...
                self.drag_accumulator -= (steps * 15)
